...
check_system() {
    if [[ "${ID}" == "centos" && ${VERSION_ID} -ge 7 ]]; then
        ...
        echo -e "${Error} ${RedBG} The current system is ${ID} ${VERSION_ID} Not in the list of supported systems, installation was interrupted ${Font}"
        exit 1
    fi
...
is_root() {
    if [ 0 == $UID ]; then
        echo -e "${OK} ${GreenBG} The current user is the root user, entering installation process ${Font}"
        sleep 3
    else
...
chrony_install() {
    ...
    chronyc tracking -v
    date
    read -rp "Please confirm whether the time is accurate(Y/N): " chrony_install
...
port_alterid_set() {
    if [[ "on" != "$old_config_status" ]]; then
        read -rp "Please enter the desired connection port(default:443）:" port
        [[ -z ${port} ]] && port="443"
        read -rp "Please enter your desired alterID（default:2）:" alterID
        [[ -z ${alterID} ]] && alterID="2"
    fi
}
modify_path() {
...
domain_check() {
    ...
    echo -e "Domain Name IP：${domain_ip}"
    echo -e "Local IP: ${local_ip}"
    sleep 2
    if [[ $(echo "${local_ip}" | tr '.' '+' | bc) -eq $(echo "${domain_ip}" | tr '.' '+' | bc) ]]; then
        echo -e "${OK} ${GreenBG} Domain Name IP matches the Local IP ${Font}"
        sleep 2
    else
        echo -e "${Error} ${RedBG} Please make sure that the correct A record is added to the domain name ${Font}"
        echo -e "${Error} ${RedBG} Domain Name IP does not match the Local IP, Do you want to continue?(Y/N) ${Font}" && read -r install
        case $install in
        [yY][eE][sS] | [yY])
            echo -e "${GreenBG} Continue to install ${Font}"
            sleep 2
            ;;
        *)
            echo -e "${RedBG} Terminate Installation ${Font}"
            exit 2
            ;;
        esac
    fi
...
port_exist_check() {
    if [[ 0 -eq $(lsof -i:"$1" | grep -i -c "listen") ]]; then
        ...
        echo -e "${Error} ${RedBG} It is detected that the $1 port is occupied, information about the $1 port ${Font}"
        lsof -i:"$1"
        echo -e "${OK} ${GreenBG} Will try to automatically kill the occupied process after 5s ${Font}"
        sleep 5
        lsof -i:"$1" | awk '{print $2}' | grep -v "PID" | xargs kill -9
        echo -e "${OK} ${GreenBG} kill success ${Font}"
        sleep 1
    fi
}
acme() {
    ...
    if "$HOME"/.acme.sh/acme.sh --issue -d "${domain}" --standalone -k ec-256 --force; then
        echo -e "${OK} ${GreenBG} The SSL certificate is generated successfully ${Font}"
        sleep 2
        mkdir /data
...
old_config_exist_check() {
    if [[ -f $v2ray_qr_config_file ]]; then
        echo -e "${OK} ${GreenBG} The old configuration file is detected, retain the old file configuration [Y/N]? ${Font}"
        read -r ssl_delete
        case $ssl_delete in
        [yY][eE][sS] | [yY])
...
tls_type() {
    if [[ -f "/etc/nginx/sbin/nginx" ]] && [[ -f "$nginx_conf" ]] && [[ "$shell_mode" == "ws" ]]; then
        ...
        echo "If you are using Quantaumlt X / router / old Shadowrocket / V2ray core lower than 4.18.1, please select compatibility mode"
        ...
        echo -e "${Error} ${RedBG} Nginx or configuration file does not exist or the currently installed version is h2 ${Font}"
    fi
}
show_access_log() {
...
uninstall_all() {
    ...
    systemctl daemon-reload
    echo -e "${OK} ${GreenBG} Uninstalled, SSL certificate file is retained ${Font}"
}
...
update_sh() {
    ...
    if [[ "$shell_version" < "$(sort -rV $version_cmp | head -1)" ]]; then
        ...
        [yY][eE][sS] | [yY])
            ...
            echo -e "${OK} ${GreenBG} Updated successfully ${Font}"
            exit 0
            ;;
        *) ;;
...
menu() {
    ...
    echo -e "${Green}7.${Font}  Change TLS Version(valid only for ws+tls)"
    ...
    echo -e "${Green}11.${Font} Install 4in1 bbr sharp speed installation script"
    ...
    read -rp "Please enter your response：" menu_num
    case $menu_num in
    0)
        update_sh
        ;;
